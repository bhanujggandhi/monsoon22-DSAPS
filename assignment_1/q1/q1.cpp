...
string remainder(string num1, string num2) {
    if (isEqual(num1, num2)) return "0";

    if (isGreater(num2, num1)) return num1;
    ...
    while (ind < num1.size()) {
        ...
        if (isGreater(num2, remain)) {
            if (ind >= num1.size()) return normalize(remain);
            remain += num1[ind++];
            quotient += '0';
        }
        int q = 0;
        while (isGreater(remain, num2) or isEqual(remain, num2)) {
...
string solveexpression(string &s) {
    ...
    int idx = 0;
    for (int i = 0; i < n; i++) {
        if (s[i] != '+' and s[i] != '-' and s[i] != 'x') continue;
    ...
    idx = 0;
    for (int i = 0; i < count; i++)
        if (expressionarr[i] != "$") exprarr[idx++] = expressionarr[i];
...
string bigintgcd(string num1, string num2) {
    if (isZero(num1) && isZero(num2)) return "Undefined GCD";
    if (isZero(num1)) return num2;
    if (isZero(num2)) return num1;

    if (isEqual(num1, num2)) return num1;
    if (remainder(num1, num2) == "0") return num2;
